...
В `src/content.config.ts` определены четыре Astro Content Collections:
...
прямой хронологической сортировки от первой опубликованной заметки к более поздним, но на сайте
отображается только дата. Необязательное поле `updated` можно добавить при изменении заметки; оно
также хранит дату и время с часовым поясом.
