...
Раздел `/lists` пока содержит ссылки на будущие страницы. Они задаются в `src/lib/lists.ts`:
...
Когда у списков появятся собственные страницы и содержимое, данные можно перенести в отдельную
Content Collection.
