...
Главная страница редактируется в `src/content/pages/home.md`: поле `lead`
управляет текстом под именем в первом экране, а необязательное поле `personal` выводит вторую
строку под ним. Страницы `/notes`, `/career`,
